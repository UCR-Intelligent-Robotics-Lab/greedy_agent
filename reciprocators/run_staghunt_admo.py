...
class GridworldStagHunt:
    ...
    def step(self, actions_tensor):
        ...
        for i in range(2):
            if actions[i] == 0: self.state[2*i+1] -= 1
            elif actions[i] == 1: self.state[2*i+1] += 1
            elif actions[i] == 2: self.state[2*i] -= 1
            elif actions[i] == 3: self.state[2*i] += 1
...
def main():
    ...
    exp_name = f"staghunt_admo_{args.num}"
    ...
    print(f"Starting ADMO ({args.mode}) training on Spatial Stag Hunt...")
    episode_count = 0
    step_train = 0
    step = 0
    last_obs = env.reset()
