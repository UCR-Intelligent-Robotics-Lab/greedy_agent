#!/usr/bin/env python3
"""Health + sanity report for completed IPD baseline runs (read-only)."""
...
# Reuse CSV helpers from ER verifier
...
def print_schema(lio_path: str, eia_path: str) -> None:
    print("=" * 70)
    print("STEP 0 — SCHEMA DISCOVERY")
    print("=" * 70)
    for label, path in [("LIO ipd_lio", lio_path), ("EIA ipd_eia", eia_path)]:
        ...
        print(f"Per-agent energy: (none in IPD logs)")
...
def analyze_run(
    ...
    rec.n_rows = len(rows)
    rec.status = "OK" if rec.n_rows == EXPECTED_ROWS else ("MISSING" if rec.n_rows == 0 else "SHORT")
    ...
    pw = env_mat[-(FINAL_WINDOW + PREV_WINDOW) : -FINAL_WINDOW] if n >= FINAL_WINDOW + PREV_WINDOW else env_mat[: max(1, n // 2)]
...
def agg_method(records: list[RunRecord], method: str) -> dict[str, Any] | None:
    rs = [
        r
        for r in records
        if r.method == method and r.status == "OK" and r.final_mean_env is not None
    ]
    if not rs:
        return None
    return {
...
def verdict_coop(lio: dict, eia: dict) -> str:
    """EIA should reduce cooperation (lower env reward and/or coop rate)."""
    signals = 0
    total = 0
...
def verdict_gap(lio: dict, eia: dict) -> str:
    """EIA should increase payoff gap."""
...
def main() -> int:
    ...
    lio_sample = os.path.join(REPO_ROOT, "lio/results/nano_ipd1/ipd_lio/log.csv")
    eia_sample = os.path.join(REPO_ROOT, "lio/results/nano_ipd1/ipd_eia/log.csv")
    print_schema(lio_sample, eia_sample)
    ...
    print(f"Expected: {len(IPD_DIRS)} dirs × {len(SEEDS)} seeds = {len(IPD_DIRS)*len(SEEDS)}")
    print(f"OK={ok}  SHORT={short}  MISSING={missing}  NaN/inf={len(nan_runs)}")

    for method in ("lio", "eia"):
        cnt = sum(1 for r in records if r.method == method and r.status == "OK")
        flag = "" if cnt == 10 else f"  *** {method} <10 ({cnt}/10) ***"
        print(f"  {method}: {cnt}/10 OK{flag}")
    ...
    lio = agg_method(records, "lio")
    eia = agg_method(records, "eia")
    ...
    if lio and eia:
        ...
        print(
            ...
            f"Var={eia['mean_fairness_var']:.1f}±{eia['std_fairness_var']:.1f}"
        )
    ...
    run_fields = [
        "seed_dir", "run_dir", "method", "n_rows", "expected", "status", "nan_inf",
        "final_mean_env", "final_coop_rate", "final_payoff_gap", "final_fairness_var",
    ...
    sum_fields = [
        "group", "method", "n_seeds", "mean_env", "std_env",
        "mean_coop_rate", "std_coop_rate", "mean_payoff_gap", "std_payoff_gap",
        "mean_fairness_var", "std_fairness_var",
    ]
    ...
    print("=" * 70)
    print(f"Complete: {ok}/20 OK ({short} SHORT, {missing} MISSING)")
    ...
    return 0 if ok == 20 and missing == 0 else 1
