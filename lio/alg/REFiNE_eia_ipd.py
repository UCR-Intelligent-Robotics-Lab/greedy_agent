...
class ExploitativeREFiNEIPD(object):

    def __init__(self, config, l_obs, l_action, nn, agent_name,
                 r_multiplier=3, n_agents=1, agent_id=0, energy_param=1.0):
       
        self.alg_name = 'REFiNE'
        self.l_obs = l_obs
        ...
        self.fairness_amplifier = config.Fairness_multiplier
        ...
        print(f"Initializing REFiNE exploitative agent {self.agent_name} ")
    ...
    def give_reward(self, obs, action_all, sess):
        ...
        gives rewards that condition other agents to defect more.
        ...
        # - High rewards for others defection
        # - Low rewards for cooperation
       
        for i, action in enumerate(action_all):
            if i != self.agent_id:
                if action == 1:  # defection
                    reward[i] = 2.0 * reward[i]  # Amplify rewards for defection
                elif action == 0:  # cooperation
                    reward[i] = 0.2 * reward[i]  # Minimize rewards for cooperation
